...
export default function AnalysesNgsContentAdminPage() {
  ...
  const fetchContent = async () => {
    try {
      const res = await fetch('/api/admin/analyses-ngs-content');
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error saving analyses NGS content:', error);
      alert('Ошибка сохранения');
    } finally {
      setSaving(false);
    }
      ...
      <div className="edit-header" style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        ...
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#091D33', margin: '0 0 4px' }}>
            NGS контент (Услуги и анализы)
          </h1>
          <p style={{ color: '#6B7280', margin: 0, fontSize: '14px' }}>
            Редактирование раздела NGS для страницы анализов
          </p>
        </div>
      </div>
          ...
          <div className="sidebar-section" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            >
              ...
              <p style={{ fontSize: '13px', color: '#6B7280', lineHeight: 1.6 }}>
                Здесь вы можете редактировать содержимое раздела NGS на странице &quot;Услуги и анализы&quot;.
              </p>
